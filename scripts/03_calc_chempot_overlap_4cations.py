# Calculates the overlap in oxygen chemical potential between four AO phases
...
combos = list(
    itertools.combinations(cations, 4)
...
for combo in combos:
    ...
    cation3 = combo[2]
    cation4 = combo[3]
    clean_combos.append(f"{cation1}{cation2}{cation3}{cation4}")
    ...
    overlap = min(max1, max2, max3, max4) - max(min1, min2, min3, min4)
...
df.to_csv("../data/chempot-overlap-4cation-GGA_GGA+U.csv", index=False)
