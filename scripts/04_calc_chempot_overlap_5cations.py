# Calculates the overlap in oxygen chemical potential between five AO phases
...
combos = list(
    itertools.combinations(cations, 5)
...
for combo in combos:
    ...
    cation4 = combo[3]
    cation5 = combo[4]
    clean_combos.append(f"{cation1}{cation2}{cation3}{cation4}{cation5}")
    ...
    overlap = min(max1, max2, max3, max4, max5) - max(min1, min2, min3, min4, min5)
...
df.to_csv("../data/chempot-overlap-5cation-GGA_GGA+U.csv", index=False)
